...
def ex1(session):
    ...
    print "\nExample#1: List the names of the servers and " \
...
# server group by using dictionary indexing. You can add a
# print list_of_server_groups to see what information, i.e.
# key/value pairs are available, so that you can access them.
...
def ex3(session):
    ...
    print "List of firewall policies:"
...
# We use the JSON load() method to place the policy into the
# variable called "data", and then we make a create() method call
# using our ConfigurationPolicy object to load the JSON-formatted
# data that is in the "data" variable into Halo.
#
...
def main():
    ...
    # 'WIN-70VB0RARSAA' is the name of the server in this example.
    # "0 CW" and "Amazon Linux Test" are the names of server
    # groups in this example. To get this to work you will have
    # to change these names to names that exist in your Halo
    # account.
    # ex7(api_session, 'WIN-70VB0RARSAA', 'Amazon Linux Test')

    # List the serverrs to show that the server was moved.
    # ex1(api_session)

    # Move a server to a given server group.
    # ex7(api_session, 'WIN-70VB0RARSAA', '0 CW')
